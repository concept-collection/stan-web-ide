...
// Ported from stan-playground's monacoStanLanguage.ts, which is adapted in
// part from https://github.com/WardBrian/vscode-stan-extension/blob/main/lang/syntaxes/stan.json
...
const SPECIAL_FUNCTIONS = ["print", "reject", "fatal_error"];
...
export const conf: monaco.languages.LanguageConfiguration = {
  ...
  folding: {
    markers: {
      start:
        /^[functions|data|transformed\s+data|parameters|transformed\s+parameters|model|generated\s+quantities]\s\{$/,
      end: /^\}$/,
    },
  },
...
export const language = <monaco.languages.IMonarchLanguage>{
  ...
  blocks: BLOCKS.join("|"),
  functions: [...FUNCTIONS, ...SPECIAL_FUNCTIONS].join("|"),
  ...
  tokenizer: {
    root: [
      [/(@blocks)\s*{/, "attribute.value.$0"],

      [/(@functions)\s*\(/, "tag.function.$0"],
      [/~\s*(@distributions)\s*\(/, "tag.distribution.$1"],
      ...
      [
        /[a-zA-Z_]\w*/,
        {
          cases: {
            "@types": "type.$0",
            "@constraints": "variable.predefined.$0",
            "@keywords": "keyword.$0",
            "@default": "identifier",
          },
        },
